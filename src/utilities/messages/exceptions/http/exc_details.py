...
def http_400_transaction_details(transaction_id) -> str:
    return f"Transaction {transaction_id} failed!"
...
def http_403_balance_details() -> str:
    return "Insufficient balance to complete the transaction!"

def http_412_task_not_complete_details(task_id: int) -> str:
    return f"Task with id {task_id} is not complete!"
